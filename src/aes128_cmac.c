#include "aes128_cmac.h"
#include <string.h>

static const uint8_t sbox[256] = {
    
};

static const uint8_t rcon[11] = {
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};

static inline uint8_t xtime(uint8_t x)
{
    return (uint8_t)((x << 1) ^ (((x >> 7) & 1) * 0x1B));
}

void aes128_init(aes128_ctx_t *ctx, const uint8_t *key)
{
    uint8_t *w = ctx->round_keys;
    memcpy(w, key, 16);

    for (uint8_t i = 4; i < 44; i++) {
        uint8_t temp[4];
        temp[0] = w[(i - 1) * 4 + 0];
        temp[1] = w[(i - 1) * 4 + 1];
        temp[2] = w[(i - 1) * 4 + 2];
        temp[3] = w[(i - 1) * 4 + 3];

        if (i % 4 == 0) {
            uint8_t k = temp[0];
            temp[0] = sbox[temp[1]] ^ rcon[i / 4];
            temp[1] = sbox[temp[2]];
            temp[2] = sbox[temp[3]];
            temp[3] = sbox[k];
        }

        w[i * 4 + 0] = w[(i - 4) * 4 + 0] ^ temp[0];
        w[i * 4 + 1] = w[(i - 4) * 4 + 1] ^ temp[1];
        w[i * 4 + 2] = w[(i - 4) * 4 + 2] ^ temp[2];
        w[i * 4 + 3] = w[(i - 4) * 4 + 3] ^ temp[3];
    }
}

void aes128_encrypt_block(const aes128_ctx_t *ctx, const uint8_t *in, uint8_t *out)
{
    uint8_t state[16];
    const uint8_t *w = ctx->round_keys;

    for (uint8_t i = 0; i < 16; i++) {
        state[i] = in[i] ^ w[i];
    }

    for (uint8_t round = 1; round < 10; round++) {
        for (uint8_t i = 0; i < 16; i++) {
            state[i] = sbox[state[i]];
        }

        uint8_t t0 = state[1];
        state[1] = state[5];
        state[5] = state[9];
        state[9] = state[13];
        state[13] = t0;

        uint8_t t1 = state[2];
        state[2] = state[10];
        state[10] = t1;
        t1 = state[6];
        state[6] = state[14];
        state[14] = t1;

        uint8_t t2 = state[15];
        state[15] = state[11];
        state[11] = state[7];
        state[7] = state[3];
        state[3] = t2;

        for (uint8_t c = 0; c < 4; c++) {
            uint8_t a0 = state[c * 4 + 0];
            uint8_t a1 = state[c * 4 + 1];
            uint8_t a2 = state[c * 4 + 2];
            uint8_t a3 = state[c * 4 + 3];

            state[c * 4 + 0] = xtime(a0 ^ a1) ^ a1 ^ a2 ^ a3;
            state[c * 4 + 1] = xtime(a1 ^ a2) ^ a2 ^ a3 ^ a0;
            state[c * 4 + 2] = xtime(a2 ^ a3) ^ a3 ^ a0 ^ a1;
            state[c * 4 + 3] = xtime(a3 ^ a0) ^ a0 ^ a1 ^ a2;
        }

        const uint8_t *rw = &w[round * 16];
        for (uint8_t i = 0; i < 16; i++) {
            state[i] ^= rw[i];
        }
    }

    for (uint8_t i = 0; i < 16; i++) {
        state[i] = sbox[state[i]];
    }

    uint8_t t0 = state[1];
    state[1] = state[5];
    state[5] = state[9];
    state[9] = state[13];
    state[13] = t0;

    uint8_t t1 = state[2];
    state[2] = state[10];
    state[10] = t1;
    t1 = state[6];
    state[6] = state[14];
    state[14] = t1;

    uint8_t t2 = state[15];
    state[15] = state[11];
    state[11] = state[7];
    state[7] = state[3];
    state[3] = t2;

    const uint8_t *rw = &w[160];
    for (uint8_t i = 0; i < 16; i++) {
        out[i] = state[i] ^ rw[i];
    }
}

static void gf_mulx(const uint8_t *in, uint8_t *out)
{
    uint8_t overflow = 0;
    for (int8_t i = 15; i >= 0; i--) {
        out[i] = (uint8_t)((in[i] << 1) | overflow);
        overflow = (in[i] & 0x80) ? 1 : 0;
    }
    if (in[0] & 0x80) {
        out[15] ^= 0x87;
    }
}

void aes128_cmac_init(aes128_cmac_ctx_t *ctx, const uint8_t *key)
{
    uint8_t zero[16] = {0};
    uint8_t l[16];

    aes128_init(&ctx->aes_ctx, key);
    aes128_encrypt_block(&ctx->aes_ctx, zero, l);
    gf_mulx(l, ctx->k1);
    gf_mulx(ctx->k1, ctx->k2);
}

void aes128_cmac_compute(const aes128_cmac_ctx_t *ctx, const uint8_t *msg, size_t len, uint8_t *mac)
{
    size_t n = (len + 15) / 16;
    if (n == 0) {
        n = 1;
    }

    uint8_t m_last[16];
    bool is_complete = (len > 0) && (len % 16 == 0);

    if (is_complete) {
        for (uint8_t i = 0; i < 16; i++) {
            m_last[i] = msg[(n - 1) * 16 + i] ^ ctx->k1[i];
        }
    } else {
        size_t rem = len % 16;
        memset(m_last, 0, 16);
        if (len > 0) {
            memcpy(m_last, &msg[(n - 1) * 16], rem);
        }
        m_last[rem] = 0x80;
        for (uint8_t i = 0; i < 16; i++) {
            m_last[i] ^= ctx->k2[i];
        }
    }

    uint8_t x[16] = {0};
    uint8_t y[16];

    for (size_t i = 0; i < n - 1; i++) {
        for (uint8_t j = 0; j < 16; j++) {
            y[j] = x[j] ^ msg[i * 16 + j];
        }
        aes128_encrypt_block(&ctx->aes_ctx, y, x);
    }

    for (uint8_t j = 0; j < 16; j++) {
        y[j] = x[j] ^ m_last[j];
    }
    aes128_encrypt_block(&ctx->aes_ctx, y, mac);
}

uint32_t aes128_cmac_compute_tag32(const aes128_cmac_ctx_t *ctx, const uint8_t *msg, size_t len)
{
    uint8_t mac[16];
    aes128_cmac_compute(ctx, msg, len, mac);
    return ((uint32_t)mac[0] << 24) | ((uint32_t)mac[1] << 16) | ((uint32_t)mac[2] << 8) | (uint32_t)mac[3];
}
